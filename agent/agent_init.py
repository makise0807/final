...
def init_agent(
    ...
    #
    # Local/custom models may intentionally run below 64K when compact prompt mode
    # is enabled. In that mode, Hermes exposes a bounded tool/skill surface and can
    # still run narrow workflows such as Geo Expert local-fixture checks.
    from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
    _ctx = getattr(agent.context_compressor, "context_length", 0)

    _provider = str(getattr(agent, "provider", "") or "").lower()
    _api_mode = str(getattr(agent, "api_mode", "") or "").lower()
    _base_url = str(getattr(agent, "base_url", "") or "").lower()

    _is_local_or_custom = (
        _provider in {"custom", "ollama", "lmstudio", "lm_studio", "local"}
        or "127.0.0.1" in _base_url
        or "localhost" in _base_url
    )

    _local_compact_prompt = (
        os.getenv("HERMES_LOCAL_COMPACT_PROMPT", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )

    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not (
        _is_local_or_custom and _local_compact_prompt
    ):
        raise ValueError(
