...
def test_pack_runner_without_user_data_is_grounded(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "No user data was provided" in str(payload["report"]["sections_map"]["User Data Evidence"]["summary"])


def test_pack_runner_geotiff_request_stays_approval_gated(tmp_path: Path, monkeypatch) -> None:
    runtime_dir = tmp_path / "user_data_runtime"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(user_data_store, "get_runtime_user_data_dir", lambda: runtime_dir)
    payload = run_pack(
        "urban_planning",
        "Need a geotiff-backed planning review.",
        {
            "aoi": {"west": 120.7, "south": 23.45, "east": 120.72, "north": 23.47},
    ...
    assert payload["success"] is True
    assert payload["openeo_acquisition"]["mode"] == "prepare_only"
    assert "openeo_submit" in payload["approval_required_actions"]
